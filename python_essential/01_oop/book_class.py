...
class Book:
    ...
    def review_info(self):
        ...
        for review in self.review_list:
            out.append(
                f'Review of "{self.name}" by: {review.user}. Text: {review.text} Time of Creation: {review.time.strftime("%H:%M  %d/%m/%Y")}')
        return '\n'.join(out)
...
def main():
    ...
    book4.add_review('Maks_Pavlenchik', 'This book is unbelievable!', datetime.datetime.now())
